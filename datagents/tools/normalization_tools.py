"""Normalization tools (A8) — make transactions comparable before matching.

Converts amounts to a base currency (exact, via Decimal), resolves entity-name
variants to a canonical name (lookup + optional LLM), and canonicalizes
reference codes. Together these let the matching step line up book vs source.
"""
...
def entity_alias_tool(name: str, gateway: LLMGateway | None = None) -> str:
    """Map a counterparty name to canonical form via lookup, LLM as fallback."""
    ...
    if gateway is not None:
        ...
        return gateway.generate(prompt).strip()
    return name.strip()
