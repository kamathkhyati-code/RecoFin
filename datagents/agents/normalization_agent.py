"""Normalization Agent (A8/A9) - apply the normalization tools to a batch.
Takes validated transactions and returns normalized copies: amounts converted
to a base currency, counterparties resolved to canonical names (via alias
table, persistent cache, or LLM), and references put in canonical form. The
output is ready for the matching step.
"""
        ...
        normalized.append(
            t.model_copy(
                update={
                    ...
                    "currency": base,
                    "counterparty": entity_alias_tool(t.counterparty, gateway, store),
...
def normalization_agent(
    ...
    *,
    ...
    normalized = normalize_transactions(txns, base=base, gateway=gateway, store=store)
